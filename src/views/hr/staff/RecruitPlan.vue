        ...
        <template>
          <span v-if="row.billState !== '30'">
            <a-popconfirm
              ...
              @confirm="approvalClick(row.id, '30')"
            >
...
import { findPage, deleteById, updateBillState } from '@/api/hr/recruitPlan'
import RecruitPlanForm from './RecruitPlanForm'
...
const columns = [
  ...
  { title: '开始日期', dataIndex: 'planDateStart', customRender: (text) => moment(text).format('YYYY-MM-DD') },
  { title: '结束日期', dataIndex: 'planDateEnd', customRender: (text) => moment(text).format('YYYY-MM-DD') },
...
const opts = {
  operators: {
    ...
    planDateStart: optEnum.equalsTo,
    planDateEnd: optEnum.equalsTo,
...
export default {
  ...
  methods: {
    approvalClick (id) {
      updateBillState(id, '30').then(res => {
        this.$refs.table.refresh()
      })
    },
    showFormClick (operateType, id) {
      this.$router.push({ name: 'recruitPlanForm', params: { id: id, operateType: operateType } })
    },
    deleteClick (id) {
